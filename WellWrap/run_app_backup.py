...
@login_required
def upload_report():
    """Upload and analyze medical reports (PDF, images, text) with timeout handling"""
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            print(f"Processing file: {filename} (Type: {determine_file_type(filename)})")
            
            # Extract text with timeout and fallback methods
            extracted_text = None
            ...
            try:
                ...
                flash(f'Error processing the uploaded file: {str(e)}', 'danger')
                # Clean up the uploaded file if it exists
                if os.path.exists(file_path):
                    os.remove(file_path)
                return redirect(request.url)
        else:
            flash('Invalid file type. Please upload PDF, image, or text files only.', 'danger')
            return redirect(request.url)
...
def determine_report_type(filename):
    ...
    if any(word in filename_lower for word in ['cbc', 'blood', 'hematology', 'hemoglobin']):
        return 'Blood Test'
    elif any(word in filename_lower for word in ['lipid', 'cholesterol']):
        return 'Lipid Panel'
    elif any(word in filename_lower for word in ['metabolic', 'chemistry', 'glucose']):
        return 'Metabolic Panel'
    elif any(word in filename_lower for word in ['thyroid', 'tsh']):
        return 'Thyroid Function'
    elif any(word in filename_lower for word in ['liver', 'alt', 'ast']):
        return 'Liver Function'
    elif any(word in filename_lower for word in ['kidney', 'creatinine']):
